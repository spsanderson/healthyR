...
line augmented to a data.frame/tibble for a set of patients.
...
An augmented data.frame/tibble with the service line appended as a new
column.

## Details

This is an augment function in that appends a vector to an
data.frame/tibble that is passed to the `.data` parameter. A
